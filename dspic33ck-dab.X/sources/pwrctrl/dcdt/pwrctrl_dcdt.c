 ...
 * @brief Data Object of voltage mode control generated by DCDT
 * 
 * @details The 'VMC_2p2z_Rev' data object holds the control loop parameter of the 
 *  Voltage loop in Reverse direction.
 *******************************************************************************/
 ...
 * @brief Data Object of current mode control
 * 
 * @details The 'IMC_2p2z_Rev' data object holds the control loop parameter of the 
 *  Current loop in Reverse direction.
 *******************************************************************************/
 ...
 * @brief Data Object of power mode control
 * 
 * @details The 'PMC_2p2z_Rev' data object holds the control loop parameter of the 
 *  Power loop in Reverse direction.
 *******************************************************************************/
SMPS_2P2Z_T PMC_2p2z_Rev; 

//==============================================================================
//==============================================================================
int16_t   VMC_pidABCCoefficients[3]__attribute__((space(xmemory)));
int16_t   VMC_pidErrorHistory[3] __attribute__((space(ymemory), far));
 ...
 *  filter for voltage mode control.
 *******************************************************************************/
int16_t   VMC_2p2zACoefficients_Rev[2]__attribute__((space(xmemory)));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Voltage Loop B-coefficient array
 * 
 ...
 *  filter for voltage mode control.
 *******************************************************************************/
int16_t   VMC_2p2zBCoefficients_Rev[3] __attribute__((space(xmemory)));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Voltage Loop Control history
 * 
 * @details This array contains the voltage loop control output history.
 *******************************************************************************/
int16_t   VMC_2p2zControlHistory_Rev[2] __attribute__((space(ymemory), far));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Voltage Loop Error history
 * 
 * @details This array contains the error history of the voltage loop control.
 *******************************************************************************/
int16_t   VMC_2p2zErrorHistory_Rev[3] __attribute__((space(ymemory), far));
 ...
 *  filter for current mode control.
 *******************************************************************************/
int16_t   IMC_2p2zACoefficients_Rev[2]__attribute__((space(xmemory)));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Current Loop B-coefficient array
 * 
 ...
 *  filter for current mode control.
 *******************************************************************************/
int16_t   IMC_2p2zBCoefficients_Rev[3] __attribute__((space(xmemory)));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Current Loop Control history
 * 
 * @details This array contains the current loop control output history.
 *******************************************************************************/
int16_t   IMC_2p2zControlHistory_Rev[2] __attribute__((space(ymemory), far));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Current Loop Error history
 * 
 * @details This array contains the error history of the current loop control.
 *******************************************************************************/
int16_t   IMC_2p2zErrorHistory_Rev[3] __attribute__((space(ymemory), far));
 ...
 *  filter for power mode control.
 *******************************************************************************/
int16_t   PMC_2p2zACoefficients_Rev[2]__attribute__((space(xmemory)));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Power Loop B-coefficient array
 * 
 ...
 *  filter for power mode control.
 *******************************************************************************/
int16_t   PMC_2p2zBCoefficients_Rev[3] __attribute__((space(xmemory)));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Power Loop Control history
 * 
 * @details This array contains the power loop control output history.
 *******************************************************************************/
int16_t   PMC_2p2zControlHistory_Rev[2] __attribute__((space(ymemory), far));
/*******************************************************************************
 * @ingroup pwrctrl-dcdt
 * @brief Power Loop Error history
 * 
 * @details This array contains the error history of the power loop control.
 *******************************************************************************/
int16_t   PMC_2p2zErrorHistory_Rev[3] __attribute__((space(ymemory), far));
...
{
  //test1  charger direction  
    
VMC_pid.abcCoefficients = &VMC_pidABCCoefficients[0];
VMC_pid.errorHistory = &VMC_pidErrorHistory[0];


VMC_pidABCCoefficients[0] = PID_PSVO_COMP_PID_COEFF_KA;
VMC_pidABCCoefficients[1] = PID_PSVO_COMP_PID_COEFF_KB;       
VMC_pidABCCoefficients[2] = PID_PSVO_COMP_PID_COEFF_KC;
        
VMC_pid.maxOutput=PID_PSVO_COMP_PID_MAX_CLAMP;
VMC_pid.minOutput = PID_PSVO_COMP_PID_MIN_CLAMP;
VMC_pid.postScaler = PID_PSVO_COMP_PID_POSTSCALER;
VMC_pid.postShift = PID_PSVO_COMP_PID_POSTSHIFT;
VMC_pid.preShift = PID_PSVO_COMP_PID_PRESHIFT;


VMC_pid.controlHistory = 0;
VMC_pidErrorHistory[0]= 0;
VMC_pidErrorHistory[1]= 0;
VMC_pidErrorHistory[2]= 0;


SMPS_ControllerPIDInitialize(&VMC_pid); 
  ...
  VMC_2p2z_Rev.aCoefficients =  &VMC_2p2zACoefficients_Rev[0]; // Set up pointer to derived coefficients
  VMC_2p2z_Rev.bCoefficients =  &VMC_2p2zBCoefficients_Rev[0]; // Set up pointer to derived coefficients
  ...
  VMC_2p2zControlHistory_Rev[1] = 0;
  VMC_2p2zErrorHistory_Rev[0] = 0;
  VMC_2p2zErrorHistory_Rev[1] = 0;
  VMC_2p2zErrorHistory_Rev[2] = 0;
   
  SMPS_Controller2P2ZInitialize(&VMC_2p2z_Rev);  
...
void PwrCtrl_IComp_Initialize(void)
{
  ...
  //explicit clear to 0
   ...
   //discharger
   IMC_2p2z_Rev.aCoefficients =  &IMC_2p2zACoefficients_Rev[0]; // Set up pointer to derived coefficients
   IMC_2p2z_Rev.bCoefficients =  &IMC_2p2zBCoefficients_Rev[0]; // Set up pointer to derived coefficients
   IMC_2p2z_Rev.controlHistory = &IMC_2p2zControlHistory_Rev[0]; // Set up pointer to controller history
   IMC_2p2z_Rev.errorHistory =   &IMC_2p2zErrorHistory_Rev[0]; // Set up pointer to error history

   IMC_2p2z_Rev.preShift =   DAB_SPIO_COMP_2P2Z_PRESHIFT;//SPIO TBD
   IMC_2p2z_Rev.postScaler = DAB_SPIO_COMP_2P2Z_POSTSCALER;
   IMC_2p2z_Rev.postShift =  DAB_SPIO_COMP_2P2Z_POSTSHIFT;
   IMC_2p2z_Rev.minOutput =  DAB_SPIO_COMP_2P2Z_MIN_CLAMP;
   IMC_2p2z_Rev.maxOutput =  DAB_SPIO_COMP_2P2Z_MAX_CLAMP;
   
   
   IMC_2p2zACoefficients_Rev[0] = DAB_SPIO_COMP_2P2Z_COEFF_A1;
   IMC_2p2zACoefficients_Rev[1] = DAB_SPIO_COMP_2P2Z_COEFF_A2;
   IMC_2p2zBCoefficients_Rev[0] = DAB_SPIO_COMP_2P2Z_COEFF_B0;
   IMC_2p2zBCoefficients_Rev[1] = DAB_SPIO_COMP_2P2Z_COEFF_B1;
   IMC_2p2zBCoefficients_Rev[2] = DAB_SPIO_COMP_2P2Z_COEFF_B2;
  
  //explicit clear to 0
   IMC_2p2zControlHistory_Rev[0] =0;
   IMC_2p2zControlHistory_Rev[1] =0;
   IMC_2p2zErrorHistory_Rev[0] =0;
   IMC_2p2zErrorHistory_Rev[1] =0;
   IMC_2p2zErrorHistory_Rev[2] =0;
   
   SMPS_Controller2P2ZInitialize(&IMC_2p2z_Rev);
}
...
void PwrCtrl_PComp_Initialize(void)  //only battery has power sense option. should be bidirectional
{
   ...
   //discharger
   PMC_2p2z_Rev.aCoefficients =  &PMC_2p2zACoefficients_Rev[0]; // Set up pointer to derived coefficients
   PMC_2p2z_Rev.bCoefficients =  &PMC_2p2zBCoefficients_Rev[0]; // Set up pointer to derived coefficients
   PMC_2p2z_Rev.controlHistory = &PMC_2p2zControlHistory_Rev[0]; // Set up pointer to controller history
   PMC_2p2z_Rev.errorHistory =   &PMC_2p2zErrorHistory_Rev[0]; // Set up pointer to error history

   PMC_2p2z_Rev.preShift =   DAB_SPPO_COMP_2P2Z_PRESHIFT;
   PMC_2p2z_Rev.postScaler = DAB_SPPO_COMP_2P2Z_POSTSCALER;
   PMC_2p2z_Rev.postShift =  DAB_SPPO_COMP_2P2Z_POSTSHIFT;
   PMC_2p2z_Rev.minOutput =  DAB_SPPO_COMP_2P2Z_MIN_CLAMP;
   PMC_2p2z_Rev.maxOutput =  DAB_SPPO_COMP_2P2Z_MAX_CLAMP; 
   
   PMC_2p2zACoefficients_Rev[0] = DAB_SPPO_COMP_2P2Z_COEFF_A1;
   PMC_2p2zACoefficients_Rev[1] = DAB_SPPO_COMP_2P2Z_COEFF_A2;
   PMC_2p2zBCoefficients_Rev[0] = DAB_SPPO_COMP_2P2Z_COEFF_B0;
   PMC_2p2zBCoefficients_Rev[1] = DAB_SPPO_COMP_2P2Z_COEFF_B1;
   PMC_2p2zBCoefficients_Rev[2] = DAB_SPPO_COMP_2P2Z_COEFF_B2;
  
    //explicit clear to 0
   PMC_2p2zControlHistory_Rev[0] =0;
   PMC_2p2zControlHistory_Rev[1] =0;
   PMC_2p2zErrorHistory_Rev[0] =0;
   PMC_2p2zErrorHistory_Rev[1] =0;
   PMC_2p2zErrorHistory_Rev[2] =0;
   
   SMPS_Controller2P2ZInitialize(&PMC_2p2z_Rev);  
